...
def parse_args():
    ...
    parser.add_argument("--dataset_json", type=str, default="datasets/DifficultCodeGeneration/DifficultCodeGeneration.jsonl")
    parser.add_argument("--result_file", type=str, default=None)
    parser.add_argument("--llm", type=str, default="vllm")
    parser.add_argument("--learn_negative_sample", type=bool, default=False)
    parser.add_argument("--learn_positive_sample", type=bool, default=False)
...
async def main():
    ...
    result_file = result_dir / f"{'' if args.learn_negative_sample else 'not'}_learn_negative_sample_{'' if args.learn_positive_sample else 'not'}_learn_demo_{args.llm}_{current_time}.json"
    ...
    for i, item in enumerate(dataloader(dataset)):
        ...
        if i % opt_frequency == opt_frequency - 1 and (args.learn_negative_sample or args.learn_positive_sample):
            tasks = [optimize(node, args.learn_positive_sample, args.learn_negative_sample) for node in agent.nodes.values() if isinstance(node, OptimizableOperation)]
